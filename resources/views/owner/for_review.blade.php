...
<head>
    ...
    <title>For Review - Owner - Document Tracking System</title>
...
<body>
            ...
            <div class="sidebar-nav">
                ...
                <div class="nav-section">
                    ...
                    <div class="nav-menu">
                        ...
                        <button class="nav-btn active" data-page="for-review" onclick="navigateTo('for-review')"><i class="bi bi-clipboard-check" style="font-size: 18px;"></i><span>For Review</span></button>
            ...
            <div class="content-body">
                ...
                <div class="documents-table-container">
                    <table class="documents-table">
                        <thead>
                            <tr>
                                <th>Code</th><th>Type</th><th>Description</th><th>Purpose</th><th>Receiving Department</th><th>File</th><th>Status</th><th>Actions</th>
                            </tr>
                        </thead>
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    @if($doc->file_data || $doc->id)
                                        <a href="#" class="view-document-link" data-document-id="{{ $doc->id }}" data-bs-toggle="modal" data-bs-target="#documentDetailsModal">View</a> | 
                                        <a href="{{ route('documents.file', $doc) }}" target="_blank" download>Download</a>
                                    @else
                                        —
                                    @endif
                                </td>
                                ...
                                <td>
                                    ...
                                    <button class="btn btn-sm btn-success me-1" onclick="acceptDocument({{ $doc->id }})" title="Accept"><i class="bi bi-check-circle"></i></button>
                                    <button class="btn btn-sm btn-outline-danger" onclick="declineDocument({{ $doc->id }})" title="Decline"><i class="bi bi-x-circle"></i></button>
                                </td>
                            </tr>
                            @empty
                            <tr><td colspan="8" class="text-center">No documents for review.</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
    ...
    async function declineDocument(docId) {
        showInputModal(
            'Decline Document',
            'Please provide a reason for declining this document (optional):',
            'Reason',
            'Enter reason (optional)',
            async (reason) => {
                try {
                    await postJson(`/documents/${docId}/decline-review`, { reason: reason || '' });
                    showToast('Document declined');
                    location.reload();
                } catch (err) {
                    showToast('Failed to decline document', 'error');
                }
            },
            'Decline',
            'btn-danger'
    ...
    function showConfirmationModal(title, message, confirmCallback, confirmText = 'Confirm', confirmClass = 'btn-primary') {
        ...
        if (!modalEl) {
            const modalHtml = `<div class="modal fade" id="confirmationModal" tabindex="-1" aria-hidden="true">
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
    ...
    async function acceptDocument(docId) {
        showConfirmationModal(
            'Accept Document',
            'Accept and mark this document as completed?',
            async () => {
                try {
                    await postJson(`/documents/${docId}/accept-review`, {});
                    showToast('Document accepted and marked as completed');
                    location.reload();
                } catch (err) {
                    console.error(err);
