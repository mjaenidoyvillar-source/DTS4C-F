...
<head>
    ...
    <title>Released - Owner - Document Tracking System</title>
...
<body>
    <div class="dashboard-container">
        <div class="sidebar">
            ...
            <div class="sidebar-nav">
                ...
                <div class="nav-section">
                    ...
                    <div class="nav-menu">
                        <button class="nav-btn active" data-page="released" onclick="navigateTo('released')"><i class="bi bi-send" style="font-size: 18px;"></i><span>Released</span></button>
                        ...
                        <button class="nav-btn" data-page="archived" onclick="navigateTo('archived')"><i class="bi bi-archive" style="font-size: 18px;"></i><span>Archived</span></button>
        ...
        <div class="main-content">
            ...
            <div class="content-body">
                <h1 class="page-title">RELEASED</h1>
                <div class="documents-table-container">
                    <table class="documents-table">
                        <thead>
                            <tr>
                                <th>Code</th><th>Type</th><th>Description</th><th>Purpose</th><th>Receiving Department</th><th>Status</th><th>Action</th>
                            </tr>
                        </thead>
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    @if(isset($doc->can_be_edited) && $doc->can_be_edited)
                                        <button class="btn btn-sm btn-outline-primary" onclick="openUpdateModal({{ $doc->id }}, '{{ $doc->title }}', '{{ $doc->document_type }}', '{{ addslashes($doc->description ?? '') }}', '{{ $doc->purpose }}', {{ $doc->receiving_department_id ?? 'null' }})" title="Update"><i class="bi bi-pencil"></i></button>
                                    @else
                                        <button class="btn btn-sm btn-outline-secondary" disabled title="Cannot edit - document has been sent to another handler"><i class="bi bi-pencil"></i></button>
                                    @endif
                                </td>
                            </tr>
                            @empty
    ...
    <div class="modal fade" id="updateDocumentModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form method="POST" id="updateDocumentForm">
                    @csrf
                    <div class="modal-body">
                        <div class="mb-3">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="document_type" id="update_document_type" class="form-control" required />
                        </div>
                        <div class="mb-3">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="purpose" id="update_purpose" class="form-control" required />
                        </div>
                        <div class="mb-3">
                            <label class="form-label">Receiving Department</label>
                            <select name="receiving_department_id" id="update_receiving_department_id" class="form-select" required>
                                <option value="" selected disabled>Select department</option>
                                @foreach(\App\Models\Department::orderBy('name')->get()->reject(function($dept) { return strtolower($dept->name) === 'administration'; }) as $dept)
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
                        <button type="submit" class="btn btn-primary"><i class="bi bi-pencil me-1"></i>Update Document</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    document.getElementById('editProfileForm')?.addEventListener('submit', async function(e) {
        ...
        function openUpdateModal(docId, title, docType, description, purpose, receivingDeptId) {
            document.getElementById('updateDocumentForm').action = `/documents/${docId}/update`;
            document.getElementById('update_title').value = title;
            document.getElementById('update_document_type').value = docType;
            document.getElementById('update_description').value = description || '';
            document.getElementById('update_purpose').value = purpose;
            if (receivingDeptId) {
                document.getElementById('update_receiving_department_id').value = receivingDeptId;
            }
            const modal = new bootstrap.Modal(document.getElementById('updateDocumentModal'));
            modal.show();
        }

        document.getElementById('updateDocumentForm').addEventListener('submit', async function(e) {
            e.preventDefault();
            const form = this;
            ...
            try {
                ...
                console.error(error);
                showToast('An error occurred while updating the document', 'error');
            }
        });
            ...
            try {
                ...
                contentDiv.innerHTML = `
                    <div class="mb-4">
                        ...
                        <p class="text-muted mb-0"><i class="bi bi-file-earmark"></i> Document Code: #${doc.id}</p>
                    </div>
                    <div class="row">
                        <div class="col-md-6 mb-3"><strong>Document Type:</strong><br><span>${doc.document_type || '—'}</span></div>
                        <div class="col-md-6 mb-3"><strong>Status:</strong><br><span class="badge bg-primary">${doc.current_status || '—'}</span></div>
                        <div class="col-md-6 mb-3"><strong>Purpose:</strong><br><span>${doc.purpose || '—'}</span></div>
                        <div class="col-md-6 mb-3"><strong>Created Date:</strong><br><span>${doc.created_at || '—'}</span></div>
                        <div class="col-md-6 mb-3"><strong>Department:</strong><br><span>${doc.department || '—'}</span></div>
                        <div class="col-md-6 mb-3"><strong>Receiving Department:</strong><br><span>${doc.receiving_department || '—'}</span></div>
                    </div>
                    <div class="mb-3">
                        <strong>Description:</strong>
                        <div class="mt-2 p-3 bg-light border-start border-primary border-4 rounded">${doc.description || 'No description provided.'}</div>
                    </div>
                    ${doc.has_file ? `
                    <div class="p-4 bg-info bg-opacity-10 border border-info rounded mt-4">
                        <h5 class="mb-3"><i class="bi bi-file-earmark-text"></i> File Information</h5>
                        <div class="row">
                            <div class="col-md-6 mb-3"><strong>File Name:</strong><br><span>${doc.file_name || '—'}</span></div>
                            <div class="col-md-6 mb-3"><strong>File Type:</strong><br><span>${doc.file_mime || '—'}</span></div>
                            ${doc.file_size ? `<div class="col-md-6 mb-3"><strong>File Size:</strong><br><span>${fileSize}</span></div>` : ''}
                        </div>
                        <div class="text-center mt-3"><p class="text-muted mb-0">Download the file to your device to view its contents</p></div>
                    </div>
                    ` : '<div class="alert alert-warning mt-4"><i class="bi bi-exclamation-triangle"></i> No file attached.</div>'}
                `;
