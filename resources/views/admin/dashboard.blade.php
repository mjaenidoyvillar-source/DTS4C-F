...
<head>
    ...
    <style>
        ...
        /* Center stats grid for admin dashboard with even spacing */
        .stats-grid {
            display: grid;
...
<body>
        ...
        <div class="sidebar">
            ...
            <div class="sidebar-nav">
                <div class="nav-section">
                    ...
                    <div class="nav-menu">
                        <button type="button" class="nav-btn active" data-page="dashboard" onclick="navigateTo('dashboard')">
                ...
                <div class="nav-section">
                    ...
                    <div class="nav-menu">
                        <button type="button" class="nav-btn" data-page="logs" onclick="navigateTo('logs')">
                            <i class="bi bi-list-ul" style="font-size: 18px;"></i>
                            <span>Audit Logs</span>
                        </button>
                    </div>
                </div>
                <!-- Admin only sees dashboard, audit logs, and user management -->
                ...
                <div class="dropdown w-100">
                    <button class="user-info w-100 bg-transparent border-0 text-start d-flex align-items-center" id="userMenuButton" data-bs-toggle="dropdown" aria-expanded="false">
                        <div class="user-avatar">
                            <img src="{{ Auth::user()->profile_picture_url ?? 'https://api.dicebear.com/7.x/avataaars/svg?seed=' . (Auth::id() ?? 0) }}" alt="{{ Auth::user()->name ?? 'User' }} avatar">
                        </div>
        ...
        <div class="main-content">
            ...
            <div class="content-body" id="contentBody" style="opacity: 0; transition: opacity 0.2s ease-in;">
                ...
                <div class="stats-grid" id="stats">
                    <div class="stat-card stat-blue" onclick="navigateTo('repository')">
                        <div class="stat-number">{{ $stats['documents'] ?? 0 }}</div>
                        <div class="stat-label">TOTAL DOCUMENTS</div>
                    </div>
                    <div class="stat-card stat-green" onclick="navigateTo('logs')">
                        <div class="stat-number">{{ $stats['auditLogs'] ?? 0 }}</div>
                        <div class="stat-label">AUDIT LOGS</div>
                    </div>
                    <div class="stat-card stat-yellow" onclick="navigateTo('users')">
                        <div class="stat-number">{{ $stats['manageUsers'] ?? 0 }}</div>
                    ...
                    <div class="documents-table-container">
                        <table class="documents-table">
                            ...
                            <tbody>
                                ...
                                @php
                                    $status = strtolower(str_replace(['_', ' '], '-', $doc->current_status ?? ''));
                                    $statusClass = 'doc-status-default';
                                    if (str_contains($status, 'pending') || $status === 'registered') {
                                        if (str_contains($status, 'handler')) $statusClass = 'doc-status-pending-handler-review';
                                        elseif (str_contains($status, 'recipient-handler')) $statusClass = 'doc-status-pending-recipient-handler';
                                        elseif (str_contains($status, 'recipient-owner') || str_contains($status, 'owner')) $statusClass = 'doc-status-pending-recipient-owner';
                                        else $statusClass = 'doc-status-pending';
                                    } elseif (str_contains($status, 'handler-review') || str_contains($status, 'received-by-handler')) {
                                        $statusClass = 'doc-status-handler-review';
                                    } elseif ($status === 'received') {
                                        $statusClass = 'doc-status-received';
                                    } elseif ($status === 'sent') {
                                        $statusClass = 'doc-status-sent';
                                    } elseif ($status === 'completed' || $status === 'complete') {
                                        $statusClass = 'doc-status-completed';
                                    } elseif ($status === 'rejected') {
                                        $statusClass = 'doc-status-rejected';
                                    } elseif ($status === 'archived') {
                                        $statusClass = 'doc-status-archived';
                                    } elseif (str_contains($status, 'for-review') || str_contains($status, 'for-owner-review')) {
                                        $statusClass = 'doc-status-for-review';
                                    }
                                    $statusText = $doc->current_status ? str_replace(['_', '-'], ' ', $doc->current_status) : '—';
                                    $statusText = ucwords(strtolower($statusText));
                                @endphp
                                <tr>
                                    <td>{{ $doc->title }}</td>
                                    ...
                                    <td>{{ optional($doc->owner)->name ?? '—' }}</td>
                                    <td>
                                        @if($doc->file_data || $doc->id)
                                            <button class="btn btn-sm btn-outline-info" onclick="viewDocumentModal({{ $doc->id }})" title="View"><i class="bi bi-eye"></i></button>
                                        @else
                                            —
                                        @endif
                                    </td>
                                ...
                                @empty
                                <tr>
                                    <td colspan="8" class="text-center">No documents yet</td>
                                </tr>
                                @endforelse
                            </tbody>
                        </table>
    ...
    async function openEditProfileModal() {
        ...
        try {
            ...
            // Force-check remove button visibility for admin case
            if (data?.user) {
                const hasPic = !!(data.user.profile_picture && data.user.profile_picture !== 'null' && data.user.profile_picture !== '');
                if (typeof toggleRemoveButton === 'function') {
                    toggleRemoveButton(hasPic);
                }
            }
        } catch (error) {
    ...
    document.getElementById('editProfileForm')?.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            if (response.ok) {
                ...
                const removeButtonContainer = document.getElementById('removePictureButtonContainer');
                const previewImg = document.getElementById('profilePicturePreview');
                
                // Check if profile picture exists
                const hasProfilePicture = data.user && data.user.profile_picture && data.user.profile_picture !== null && data.user.profile_picture !== '';
                
                if (hasProfilePicture) {
                    // Update preview in modal
                    if (previewImg) {
                        previewImg.src = data.user.profile_picture + '?t=' + new Date().getTime();
                    }
                    
                    // Update avatar images on page
                    const avatarImages = document.querySelectorAll('.user-avatar img, .navbar .user-avatar img');
                    ...
                    // If picture was removed, update to default
                    if (previewImg) {
                    ...
                    const avatarImages = document.querySelectorAll('.user-avatar img, .navbar .user-avatar img');
                    avatarImages.forEach(img => {
                        img.src = `https://api.dicebear.com/7.x/avataaars/svg?seed={{ Auth::id() ?? 0 }}`;
                    });
